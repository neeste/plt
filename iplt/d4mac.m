...
#else
#include <Carbon/Carbon.h>  // Mac Carbon
#endif
#include <stdio.h>
...
static void beg_page(), end_page();
static void stroke_path(), background();
static void setrgb(), setpen(float), setclip();
...
double  pw;			/* pen width in inches */
{
    ...
    pen = limit(0.005 , pw, dev_.maxpen) * adupi;	/* force pen value into range */
}
...
background()
{
	...
	if (CGContextDrawLinearGradient != NULL) {  // use gradient, if available
	    CGPoint			p1, p2;
	    CGGradientRef		grd;
...
#else
	{		    // use strips if OSX version < 10.5
#endif
...
mac_context(CGContextRef *c, CGRect *b, CGColorSpaceRef s)
{
	...
	adupi = b->size.width / XPAGE;
	txx = tyy = adupi;
	txy = tyx = 0;
	tx0 = b->origin.x;
	ty0 = b->origin.y;
    } else {
	adupi = b->size.width / YPAGE;
	txy = tyx = adupi;
	txx = tyy = 0;
	tx0 = b->origin.x;
	ty0 = b->origin.y;
