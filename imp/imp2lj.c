/* imp2lj.c */
/* Translates impress language into HP Lasertjet graphics commands. */
...
char  **av;
{
	...
	while (av[1][0] == '-') {
		...
		if (av[2][0] == '-')
		    prn = stdout;
		else if ((prn = fopen(av[2], "wb")) == NULL)
		    cant(av[2]);
                ac--;
                av++;
		break;
...
{
    (void) printf("\n\n");
    (void) printf("program name: IMP2LJ, version 1.3\n\n");
    (void) printf("Reads a file of impress commands and outputs HP laserjet graphics to 'stdprn.'\n");
    (void) printf("When no file name arguments are present, reads impress from 'stdin'.\n\n");
    (void) printf("usage: imp2lj [options] impress_file \n\n");
    (void) printf("options:\n");
    (void) printf("    -h   	print usage information\n");
    (void) printf("    -o F 	put graphic codes in file F instead of stdprn\n");
    (void) printf("    -q   	set resolution to 300 dpi [default = 75 dpi]\n");
    (void) printf("    -rN   	set resolution to N dpi, N=75,150,300\n");
